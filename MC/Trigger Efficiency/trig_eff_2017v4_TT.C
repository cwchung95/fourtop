void trig_eff_2017v4_TT(TString var="ht", float begin=0, float end=2000, int nbins=20)
{
	/* Make Chain */
	TChain ch("outputTree");
	ch.Add("/home/cwchung95/4top20162017/2017/2017TTskimmedfiles/*.root");
	...
	TString nbdeno = " && Sum$(Jet_btagDeepB>0.4941 && abs(Jet_eta)<2.4 && Jet_pt>40)>=2 ";        
	// Make a HT denominator component //
	TString htdeno = " && Sum$((Jet_pt>40 && abs(Jet_eta) < 2.4)* Jet_pt)> 700";
	TString deno;
	// Make a trigger string //
	TString trig = "(HLT_PFHT380_SixPFJet32_DoublePFBTagCSV_2p2 || HLT_PFHT430_SixPFJet40_PFBTagCSV_1p5 || HLT_PFHT300PT30_QuadPFJet_75_60_45_40_TriplePFBTagCSV_3p0)";
	...
	if(var == "nb")
	{
		...
		var = "Sum$(Jet_btagDeepB>0.4941 && abs(Jet_eta)<2.4 && Jet_pt>40)";
		begin = -0.5;
		end = 4.5;
		nbins = 5;
		deno = indeno+htdeno;
