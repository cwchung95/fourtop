void trig_eff_2016v4_TT(TString var="ht", float begin=0, float end=2000, int nbins=20)
{
	/* Make Chain */
	TChain ch("outputTree");
	ch.Add("/home/cwchung95/4top20162017/2016/2016TTskimmedfiles/*.root");
	...
	TString nbdeno = " && Sum$(Jet_btagDeepB>0.6324 && abs(Jet_eta)<2.4 && Jet_pt>40)>=2 ";        
	...
	if(var == "nb")
	{
		...
		var = "Sum$(Jet_btagDeepB>0.6324 && abs(Jet_eta)<2.4 && Jet_pt>40)";
		begin = -0.5;
		end = 4.5;
		nbins = 5;
		deno = indeno + htdeno;
