void trig_eff_2017v4_Data_Nb_Njets_backup(TString var1="nb",TString var2="njets")
{
	/* Make Chain */
	TChain ch1("Events");
	TChain ch2("Events");
	ch1.Add("/home/cwchung95/exampleV/skim_trig/public/skim_trig_8jet_2017v4_data_SingleMuon_B.root");
	ch2.Add("/home/cwchung95/exampleV/skim_trig/public/skim_trig_8jet_2017v4_data_SingleMuon_C.root");
	ch2.Add("/home/cwchung95/exampleV/skim_trig/public/skim_trig_8jet_2017v4_data_SingleMuon_D.root");
	ch2.Add("/home/cwchung95/exampleV/skim_trig/public/skim_trig_8jet_2017v4_data_SingleMuon_E.root");
	ch2.Add("/home/cwchung95/exampleV/skim_trig/public/skim_trig_8jet_2017v4_data_SingleMuon_F.root");
	cout << "Number of Entries : " << ch1.GetEntries() + ch2.GetEntries() << endl;
	...
	TString indeno = " Sum$(Jet_jetId>1 && Jet_pt>30 && abs(Jet_eta)<2.4)>=8 "; 
	// Make a Nb denominator component //
	TString nbdeno = " && Sum$(Jet_btagDeepB>0.4941 && abs(Jet_eta)<2.4 && Jet_pt>30 && Jet_jetId>1 )>=3 ";        
	// Make a HT denominator component //
	TString htdeno = "(HLT_IsoMu27||HLT_IsoMu24) && Sum$((Jet_pt>30 && abs(Jet_eta) < 2.4 && Jet_jetId>1 )* Jet_pt) > 700";
	htdeno = htdeno + " && Sum$((Jet_pt>30 && abs(Jet_eta) < 2.4 && Jet_jetId>1)* Jet_pt) < 1000";
	TString deno[2];
	...
	for(int i=0; i<2; i++){
		...
		{
			label="HT [GeV]";
			var[i] = "Sum$((Jet_jetId>1 && Jet_pt>30 && abs(Jet_eta)<2.4)*Jet_pt)";
			begin[i] = 0;
			end[i] = 2000;
			nbins[i] = 20;
		...
		{
			label="Njets";
			var[i] = "Sum$(Jet_jetId>1 && Jet_pt>30 && abs(Jet_eta)<2.4)";
			begin[i] = 7.5;
			end[i] = 12.5;
			nbins[i] = 5;
			deno[i] = indeno + nbdeno;
		...
		{
			label="Nb";
			var[i] = "Sum$(Jet_btagDeepB>0.4941 && abs(Jet_eta)<2.4 && Jet_pt>30 && Jet_jetId>1)";
			begin[i] = 2.5;
			end[i] = 4.5;
			nbins[i] = 2;
	...
	ch1.Draw(Form("min(%s,%f-0.00001):min(%s,%f-0.00001)>>+h2_den", var[1].Data(), end[1], var[0].Data(), end[0]), "1*("+htdeno+")", "goff");
	ch2.Draw(Form("min(%s,%f-0.00001):min(%s,%f-0.00001)>>+h2_den", var[1].Data(), end[1], var[0].Data(), end[0]), "1*("+htdeno+")", "goff");
	ch1.Draw(Form("min(%s,%f-0.00001):min(%s,%f-0.00001)>>+h2_num", var[1].Data(), end[1], var[0].Data(), end[0]), "1*(("+htdeno+")&&("+trig1+"))", "goff");
	ch2.Draw(Form("min(%s,%f-0.00001):min(%s,%f-0.00001)>>+h2_num", var[1].Data(), end[1], var[0].Data(), end[0]), "1*(("+htdeno+")&&("+trig2+"))", "goff");
	...
	l1->Draw("same");
	*/
	TString outfile = xlabel+"2017v4_2D_Backup";
	c->Print(outfile+".pdf"); 
